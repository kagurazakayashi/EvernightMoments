...
| `-x`   | `--exiftool-path`  | ExifTool 実行ファイルのパスを設定（空で無効化）          | `-x "C:\Tools\exiftool.exe"`                    |
| `-r`   | `--recursive`      | サブディレクトリを再帰的に処理                           | `-r`                                            |
| `-m`   | `--multi-ext`      | 複数拡張子サポートを有効化（デフォルト）                  | `-m`                                            |
| `-nm`  | `--no-multi-ext`   | 複数拡張子サポートを無効化                               | `-nm`                                           |
...
- 「複数拡張子」トグル：オン（デフォルト）にするとすべての拡張子を除去し、中間層（例: `photo.ARW.dop` の `.ARW`）も拡張子として除去します。オフにすると最後の拡張子のみ除去し、中間層はファイル名として保持します。
