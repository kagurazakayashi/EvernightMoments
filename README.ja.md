...
1. root 権限を必要とせずに書き込み可能な場所に解凍します。
2. **端末（ターミナル）**を開き、以下のコマンドを実行します：
...
- このスクリプトは以下の操作を自動的に行います：
  - プログラムメニューとデスクトップに設定アイコンを作成します。
  - ターミナルでいつでも使用できるように、コマンドを `/.local/bin` フォルダに追加します。
    - コマンドが見つからない場合は、そのフォルダの絶対パスを各自で `PATH` 環境変数に追加する必要があります。
- **注意：一部のオペレーティングシステムでは、署名のないプログラムの実行が拒否される場合があります。**本プログラムを使用するには、署名のないプログラムの実行を許可する必要があります。通常、グラフィカルな画面上で実行が拒否された場合は、ポップアップ通知が表示されます。
