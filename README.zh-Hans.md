...
| `-x`   | `--exiftool-path`  | 设定 ExifTool 执行档路径（空字串可停用）                | `-x "C:\Tools\exiftool.exe"`                    |
| `-r`   | `--recursive`      | 递回处理子目录                                       | `-r`                                            |
| `-m`   | `--multi-ext`      | 启用多重副档名支援（预设）                               | `-m`                                            |
| `-nm`  | `--no-multi-ext`   | 停用多重副档名支援                                      | `-nm`                                           |
...
- 「多重副档名」开关：勾选时（预设）剥离所有副档名，中间层（如 `photo.ARW.dop` 中的 `.ARW`）视为副档名一并剥离。取消勾选时仅剥离最後一层副档名，中间层保留为档名。
