...
| `-x`   | `--exiftool-path`  | 設定 ExifTool 執行檔路徑（空字串可停用）               | `-x "C:\Tools\exiftool.exe"`                    |
| `-r`   | `--recursive`      | 遞迴處理子目錄                                       | `-r`                                            |
| `-m`   | `--multi-ext`      | 啟用多重副檔名支援（預設）                               | `-m`                                            |
| `-nm`  | `--no-multi-ext`   | 停用多重副檔名支援                                      | `-nm`                                           |
| `-nc`  | `--no-color`       | 停用彩色終端輸出                                       | `-nc`                                           |
...
- 「多重副檔名」開關：勾選時（預設）剝離所有副檔名，中間層（如 `photo.ARW.dop` 中的 `.ARW`）視為副檔名一併剝離。取消勾選時僅剝離最後一層副檔名，中間層保留為檔名。

### 9. 是否啟用彩色輸出？

- 「彩色輸出」開關：勾選後終端輸出會包含 ANSI 色彩碼。若重新導向輸出或透過 AI 代理使用此工具，建議取消勾選。
